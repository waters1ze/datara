...
fn main() {
    ...
    println!("cargo:rerun-if-changed=src/runtime/datara_js.c");
    println!("cargo:rerun-if-changed=src/runtime/datara_napi.h");
    println!("cargo:rerun-if-changed=src/runtime/datara_napi.c");
    println!("cargo:rerun-if-changed=src/runtime/datara_py.c");
    ...
    build
        ...
        .file(runtime_dir.join("datara_rt_scheduler.c"))
    ...
    let file_name = archive
        .file_name()
        .unwrap_or_else(|| std::ffi::OsStr::new("datara_runtime.lib"));
    if let Err(e) = std::fs::create_dir_all(&dest_runtime_dir) {
        println!(
            "cargo:warning=Failed to create runtime/ mirror directory: {}",
            e
        );
    } else if let Err(e) = std::fs::copy(&archive, dest_runtime_dir.join(file_name)) {
        println!(
    ...
    if let Ok(home) = env::var("DATARA_HOME") {
        ...
        } else if let Err(e) = std::fs::copy(&archive, home_runtime_dir.join(file_name)) {
            println!(
