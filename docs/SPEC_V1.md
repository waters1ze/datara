...
- **Normative Rule:** Standard integer operations on `Int` (signed 64-bit) execute fail-closed checked arithmetic by default: overflow (`i64::MAX + 1`, `i64::MIN / -1`) triggers a deterministic trap across all backends (Cranelift, LLVM, WebAssembly).
- **Explicit Policies:** For modular two's complement arithmetic ($Z/2^{64}$), explicit `wrapping(...)` is used. For saturating bounds arithmetic, explicit `saturating(...)` is used.
