...
def sync_version(version: str):
    ...
    version_file.write_text(clean_ver + "\n", encoding="utf-8")
    print(f"  [UPDATED] VERSION ({clean_ver})")
    ...
    # 12. packages/pypi/datara/__init__.py
    pypi_init = repo_root / "packages" / "pypi" / "datara" / "__init__.py"
    if pypi_init.exists():
        update_file(
            pypi_init,
            r'__version__\s*=\s*"[^"]+"',
            f'__version__ = "{clean_ver}"',
            "packages/pypi/datara/__init__.py"
        )

    # 13. packaging/homebrew/Formula/datara.rb
    homebrew_rb = repo_root / "packaging" / "homebrew" / "Formula" / "datara.rb"
    if homebrew_rb.exists():
        content = homebrew_rb.read_text(encoding="utf-8")
        content = re.sub(r'version\s+"[^"]+"', f'version "{clean_ver}"', content)
        content = re.sub(r'/releases/download/v[^/]+/', f'/releases/download/{tag_ver}/', content)
        homebrew_rb.write_text(content, encoding="utf-8")
        print(f"  [UPDATED] packaging/homebrew/Formula/datara.rb")

    # 14. packaging/aur/PKGBUILD
    ...
    if readme_md.exists():
        ...
        content = re.sub(r'datara-language-[0-9]+\.[0-9]+\.[0-9]+\.vsix', f'datara-language-{clean_ver}.vsix', content)
        content = re.sub(r'datara_[0-9]+\.[0-9]+\.[0-9]+_amd64\.deb', f'datara_{clean_ver}_amd64.deb', content)
        content = re.sub(r'datara-[0-9]+\.[0-9]+\.[0-9]+-1\.x86_64\.rpm', f'datara-{clean_ver}-1.x86_64.rpm', content)
        content = re.sub(r'REPL \(Zero-Latency In-Process JIT Console v[0-9]+\.[0-9]+\.[0-9]+\)', f'REPL (Zero-Latency In-Process JIT Console {tag_ver})', content)
        content = re.sub(r'badge/version-[0-9]+\.[0-9]+\.[0-9]+', f'badge/version-{clean_ver}', content)
        readme_md.write_text(content, encoding="utf-8")
        print(f"  [UPDATED] README.md")

    # 17. install.ps1 & install.sh fallbacks
