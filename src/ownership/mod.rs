            ...
            match decl {
                ...
                Decl::Impl(i) => {
                    for m in &i.methods {
                        ...
                        self.fn_signatures.insert(m.name.clone(), m.params.clone());
                    }
                }
                _ => {}
            }
        ...
        match decl {
            ...
            Decl::Impl(i) => {
                for m in &i.methods {
                    ...
                    self.bindings_by_scope.clear();
                    self.states.insert("this".to_string(), ValueState::Active);
                    self.states.insert("self".to_string(), ValueState::Active);
                    for p in &m.params {
                        self.states.insert(p.name.clone(), ValueState::Active);
                    }
                    self.check_stmt(&m.body, diag, true);
                }
            }
            _ => {}
        }
    ...
    fn check_stmt(&mut self, stmt: &Stmt, diag: &mut DiagnosticEngine, _is_root_fn: bool) {
        match stmt {
            ...
            Stmt::Assign {
                ...
                if let Expr::Identifier(name, _target_span) = target {
                    if !self.local_bindings.contains(name) {
                        self.handle_binding(name, value, false, span, diag);
                    } else {
                        // 1. Check if variable has active immutable views
                ...
                if let Some(ValueState::Moved { at_span, reason }) = self.states.get(name) {
                    ...
                    if let Some(m_borrow) = borrows.iter().find(|b| b.is_mut) {
                        diag.error_with_help(
                            ...
                            format!(
                                "Cannot read '{}' because it is actively mutably borrowed by '{}' at {}",
                                name, m_borrow.borrower, m_borrow.span
                            ),
                            Some(span.clone()),
                            Some(format!(
                                "ensure mutable view '{}' has finished its lifecycle before reading '{}'",
                                m_borrow.borrower, name
                            )),
                        );
                    }
                }
