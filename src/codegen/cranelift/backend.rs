//! Cranelift Backend for Datara.
//!
//! Note on LTO:
//! Cross-language Link-Time Optimization (LTO) across Datara and foreign native code
//! (such as Rust crates compiled with embed-bitcode) is supported strictly via the LLVM backend
//! (`--llvm --lto`). Cranelift generates native machine code directly and does not consume or produce
//! LLVM bitcode; therefore cross-language LTO across foreign staticlibs is skipped on the Cranelift path.
...
impl RealCraneliftBackend {
    ...
    pub fn compile_into_module_opt<M: ClifModule>(
        ...
        for name in &sorted_func_names {
            let f = &dmir_module.functions[name];
            let &(func_id, ref sig) = func_ids.get(name).ok_or_else(|| {
                format!("Code generation failed: function '{}' not declared", name)
            })?;
            ...
            // Values holding a map pointer produced by `datara_rt_map_create*`.
                    ...
                    match inst {
                        ...
                        Inst::BinOp {
                            ...
                            if is_string {
                                ...
                                let l_str = if left_is_str {
                                    raw_lv
                                } else if bool_vids.contains(left) {
                                    let call = builder.ins().call(bool_to_str_ref, &[raw_lv]);
                                    builder.inst_results(call)[0]
                                } else if lv_ty == clif_types::F64 {
                                    let call = builder.ins().call(flt_to_str_ref, &[raw_lv]);
                                    builder.inst_results(call)[0]
                                } else {
                                ...
                                let r_str = if right_is_str {
                                    raw_rv
                                } else if bool_vids.contains(right) {
                                    let call = builder.ins().call(bool_to_str_ref, &[raw_rv]);
                                    builder.inst_results(call)[0]
                                } else if rv_ty == clif_types::F64 {
                                    let call = builder.ins().call(flt_to_str_ref, &[raw_rv]);
                                    builder.inst_results(call)[0]
                                } else {
                            ...
                            let res = if is_float {
                                ...
                                match op.as_str() {
                                    "+" => {
                                        let (res, ovf) = builder.ins().sadd_overflow(lv, rv);
                                        builder.ins().trapnz(
                                            ovf,
                                    ...
                                    "-" => {
                                        let (res, ovf) = builder.ins().ssub_overflow(lv, rv);
                                        builder.ins().trapnz(
                                            ovf,
                                            cranelift_codegen::ir::TrapCode::INTEGER_OVERFLOW,
                                        );
                                    ...
                                    "*" => {
                                        let (res, ovf) = builder.ins().smul_overflow(lv, rv);
                                        builder.ins().trapnz(
                                            ovf,
                                            cranelift_codegen::ir::TrapCode::INTEGER_OVERFLOW,
                                        );
                                    ...
                                    "wrapping_+" => builder.ins().iadd(lv, rv),
                                    "wrapping_-" => builder.ins().isub(lv, rv),
                                    "wrapping_*" => {
                                    ...
                                    "saturating_+" => {
                                        ...
                                        let is_pos = builder.ins().icmp(
                                            cranelift_codegen::ir::condcodes::IntCC::SignedGreaterThanOrEqual,
                                            lv,
                                            zero,
                                        );
                                        let max_val =
                                            builder.ins().iconst(clif_types::I64, i64::MAX);
                                        let min_val =
                                            builder.ins().iconst(clif_types::I64, i64::MIN);
                                        let sat_val =
                                            builder.ins().select(is_pos, max_val, min_val);
                                        builder.ins().select(ovf, sat_val, res)
                                    }
                                    "saturating_-" => {
                                        let (res, ovf) = builder.ins().ssub_overflow(lv, rv);
                                        let zero = builder.ins().iconst(clif_types::I64, 0);
                                    ...
                                    "saturating_*" => {
                                        let (res, ovf) = builder.ins().smul_overflow(lv, rv);
                                        let xor_val = builder.ins().bxor(lv, rv);
                                        let zero = builder.ins().iconst(clif_types::I64, 0);
                                        let is_pos = builder.ins().icmp(
                                            cranelift_codegen::ir::condcodes::IntCC::SignedGreaterThanOrEqual,
                                            xor_val,
                                    ...
                                    "/" => {
                                        ...
                                        builder.ins().trapnz(is_zero, cranelift_codegen::ir::TrapCode::INTEGER_DIVISION_BY_ZERO);
                                        ...
                                        let neg_one = builder.ins().iconst(clif_types::I64, -1);
                                        let is_neg_one = builder.ins().icmp(
                                            cranelift_codegen::ir::condcodes::IntCC::Equal,
                                            rv,
                                            neg_one,
                                        );
                                        let is_ovf = builder.ins().band(is_min, is_neg_one);
                                        builder.ins().trapnz(
                                            is_ovf,
                                            cranelift_codegen::ir::TrapCode::INTEGER_OVERFLOW,
                                        );
                                    ...
                                    "==" => {
                                        if string_vids.contains(left)
                                            || string_vids.contains(right)
                                            || ty == "String"
                                            || ty.contains("Str")
                                        {
                                            let eq_ref = module
                                                .declare_func_in_func(rt_str_eq_id, builder.func);
                        ...
                        Inst::Call {
                            ...
                            if func == "destroy" || func == "drop" {
                                val_map.insert(*dest, builder.ins().iconst(clif_types::I64, 0));
                                continue;
                            }
                            ...
                            if (func == "float4" || func == "datara_rt_float4") && args.len() == 4 {
                                ...
                                let slot_addr = builder.ins().stack_addr(clif_types::I64, slot, 0);
                            ...
                            if (func == "dot" || func == "datara_rt_float4_dot") && args.len() == 2
                            {
                                ...
                                let l1_0 = builder.ins().load(clif_types::F32, flags, v1, 0);
                                let l2_0 = builder.ins().load(clif_types::F32, flags, v2, 0);
                                let p0 = builder.ins().fmul(l1_0, l2_0);

                                let l1_1 = builder.ins().load(clif_types::F32, flags, v1, 4);
                                let l2_1 = builder.ins().load(clif_types::F32, flags, v2, 4);
                                let p1 = builder.ins().fmul(l1_1, l2_1);

                                let l1_2 = builder.ins().load(clif_types::F32, flags, v1, 8);
                                let l2_2 = builder.ins().load(clif_types::F32, flags, v2, 8);
                                let p2 = builder.ins().fmul(l1_2, l2_2);

                                let l1_3 = builder.ins().load(clif_types::F32, flags, v1, 12);
                                let l2_3 = builder.ins().load(clif_types::F32, flags, v2, 12);
                                let p3 = builder.ins().fmul(l1_3, l2_3);

                                let s0 = builder.ins().fadd(p0, p1);
                                let s1 = builder.ins().fadd(p2, p3);
                                let sum = builder.ins().fadd(s0, s1);
                                ...
                                && let Ok(n) = rest.parse::<usize>()
                            {
                                ...
                                let size_val = builder.ins().iconst(
                                    clif_types::I64,
                                    LIST_HEADER_SIZE.saturating_add(
                                        (n.saturating_add(1) as i64).saturating_mul(8),
                                    ),
                                );
                                let call_inst = builder.ins().call(malloc_ref, &[size_val]);
                                let hdr_addr = builder.inst_results(call_inst)[0];
                                let slot_addr =
                                    builder.ins().iadd_imm_s(hdr_addr, LIST_HEADER_SIZE);
                                let flags = cranelift_codegen::ir::MachMemFlags::new();
                                let cap_val = builder.ins().iconst(clif_types::I64, n as i64);
                                let magic_val = builder.ins().iconst(clif_types::I64, LIST_MAGIC);
                                builder.ins().store(flags, cap_val, hdr_addr, 0);
                                builder.ins().store(flags, magic_val, hdr_addr, 8);
                                ...
                                None => {
                                    ...
                                    matched
                                        .or_else(|| {
                                            ...
                                            let mut candidates: Vec<&String> = func_ids
                                                .keys()
                                                .filter(|k| {
                                                    k.split_once('_')
                                                        .map(|(_, m)| m == func)
                            ...
                            for a in args {
                                ...
                                let final_av = if is_str_concat && !string_vids.contains(a) {
                                    let call = if bool_vids.contains(a) {
                                        builder.ins().call(bool_to_str_ref, &[av])
                                    } else if builder.func.dfg.value_type(av) == clif_types::F64 {
                                        builder.ins().call(flt_to_str_ref, &[av])
                                    } else {
                                        builder.ins().call(conv_ref, &[av])
                                    };
                                    builder.inst_results(call)[0]
                                } else {
                                    av
                            ...
                            if callee_name.ends_with("http_get") && arg_vals.is_empty() {
                                arg_vals.push(builder.ins().iconst(clif_types::I64, 0));
                            }
                            ...
                            if let Some((_, callee_sig)) = func_ids.get(&callee_name) {
                                for (i, av) in arg_vals.iter_mut().enumerate() {
                                    if builder.func.dfg.value_type(*av) == clif_types::F64
                                        && callee_sig
                                            .params
                                            .get(i)
                            ...
                            if let Some(&r) = results.first() {
                                ...
                                if (ty == "Float" || ret_ty == "Float")
                                    && builder.func.dfg.value_type(r) == clif_types::I64
                                {
                                    r_val = builder.ins().bitcast(
                                        clif_types::F64,
                                        cranelift_codegen::ir::MemFlagsData::new(),
                                        r,
                                    );
                                }
                                val_map.insert(*dest, r_val);
                                ...
                                if ty == "Map" || func.starts_with("datara_rt_map_create") {
                                    map_vids.insert(*dest);
                                }
                                if !ret_ty.is_empty() {
                                    val_to_class.insert(*dest, ret_ty.to_string());
                                } else if !ty.is_empty() {
                                    val_to_class.insert(*dest, ty.clone());
                                }
                            }
                        }
                        Inst::MethodCall {
                            ...
                            let list_special = if map_vids.contains(object) {
                                ...
                                // verification.
                                match method.as_str() {
                                    "get" | "at" => Some(rt_map_get_id),
                                    "insert" | "set" | "add" | "put" => Some(rt_map_insert_id),
                                    _ => None,
                                }
                            } else if list_vids.contains(object) {
                                match method.as_str() {
                                ...
                                match method.as_str() {
                                    "length" | "count" | "len" | "byte_len" => Some(str_len_id),
                                    "char_len" | "chars" => Some(str_chars_id),
                                    "byte_at" => Some(str_byte_at_id),
                                    "char_at" => Some(rt_str_char_at_id),
                                    _ => None,
                                }
                            } else {
                                None
                            ...
                            if let Some(special_id) = list_special {
                                ...
                                if let Some(&r) = results.first() {
                                    let ret_ty = dmir_module
                                        ...
                                        .map(|f| f.return_type.as_str())
                                        .unwrap_or("");
                                    let obj_class = val_to_class.get(object).cloned();
                                    let is_float = ty == "Float"
                                        ...
                                        || (ret_ty == "T"
                                            && obj_class
                                                .as_ref()
                                                .map(|c| {
                                                    c.contains("<Float>") || c.contains("<f64>")
                                                })
                                                .unwrap_or(false));
                                    let mut r_val = r;
                                    if is_float && builder.func.dfg.value_type(r) == clif_types::I64
                                    {
                                        r_val = builder.ins().bitcast(
                                            clif_types::F64,
                                            cranelift_codegen::ir::MemFlagsData::new(),
                                    ...
                                    if !effective_class.is_empty() {
                                        val_to_class.insert(*dest, effective_class);
                                    }
                                }
                            }
                        }
