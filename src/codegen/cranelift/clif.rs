...
impl<'a> ClifEmitter<'a> {
    ...
    pub fn emit_function(&self, f: &Function, module: &Module) -> Result<String, String> {
        ...
        fn collect_vars(inst: &Inst, all_vars: &mut HashSet<String>) {
            match inst {
                Inst::AssignVar { name, .. } => {
                    all_vars.insert(name.clone());
                }
        ...
        for b in &f.blocks {
            for inst in &b.instructions {
                collect_vars(inst, &mut all_vars);
            }
        }
        ...
        let code = match inst {
            ...
            Inst::BinOp {
                ...
                let clif_op = match op.as_str() {
                    ...
                    _ => {
                        return Err(format!(
                            "[E0903] unknown binary operator '{}' in CLIF printer",
                            op
                        ));
                    }
                };
            ...
            Inst::Decide {
                ...
                let else_str = else_val
                    .map(|v| format!("v{}", v.0))
                    .unwrap_or_else(|| "0".to_string());
                let mut curr_val = else_str;
                for (cond, val) in arms.iter().rev() {
                    out.push_str(&format!(
                        "    v{} = select v{}, v{}, {}\n",
                        dest.0, cond.0, val.0, curr_val
                    ));
                    curr_val = format!("v{}", dest.0);
                }
                out
            }
            Inst::WhileLoop {
    ...
    fn dmir_type_to_clif(&self, ty: &str) -> &'static str {
        match ty {
            "Int" | "Int64" | "UInt64" | "i64" | "u64" | "isize" | "usize" | "USize" => "i64",
