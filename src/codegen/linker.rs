    ...
    match spec.flavor {
        LinkerFlavor::Msvc => {
            ...
            for lib in extra_libs {
                ...
                if file_part.ends_with(".lib") || file_part.ends_with(".dll") {
                    args.push(file_part);
                } else {
                    args.push(format!("{}.lib", file_part));
                }
            }
        ...
        LinkerFlavor::Unix => {
            ...
            if std::env::var("RUSTFLAGS")
                ...
                .unwrap_or(false)
                || std::env::var("ASAN_OPTIONS").is_ok()
            {
            ...
            for lib in extra_libs {
                ...
                if let Some(parent) = p.parent() {
                    if !parent.as_os_str().is_empty() {
                        args.push(format!("-L{}", parent.display()));
                    }
                }
                let file_part = p
                    .file_name()
                ...
                // Skip Windows system libs if compiling on non-Windows host
                if !cfg!(windows)
                    && (file_part.ends_with(".lib")
                        || file_part.eq_ignore_ascii_case("kernel32")
                        || file_part.eq_ignore_ascii_case("user32")
                        || file_part.eq_ignore_ascii_case("ws2_32"))
                {
                    continue;
                }

                if file_part.starts_with("lib") && file_part.ends_with(".a") {
                    let clean = &file_part[3..file_part.len() - 2];
                    args.push(format!("-l{}", clean));
                } else if file_part.starts_with("lib") && file_part.ends_with(".so") {
                    let clean = &file_part[3..file_part.len() - 3];
                    args.push(format!("-l{}", clean));
                } else if file_part.starts_with("lib") && file_part.ends_with(".dylib") {
                    let clean = &file_part[3..file_part.len() - 6];
                    args.push(format!("-l{}", clean));
                } else if file_part.ends_with(".a")
                    || file_part.ends_with(".so")
                    || file_part.ends_with(".dylib")
                {
                    args.push(p.display().to_string());
                } else {
                    let stripped = file_part.strip_prefix("lib").unwrap_or(&file_part);
                    let clean = stripped
                        .strip_suffix(".so")
                        .or_else(|| stripped.strip_suffix(".dylib"))
                        .or_else(|| stripped.strip_suffix(".a"))
                        .or_else(|| stripped.strip_suffix(".lib"))
                        .unwrap_or(stripped);
                    args.push(format!("-l{}", clean));
                }
            }
