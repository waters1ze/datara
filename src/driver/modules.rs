...
impl ForgenCompiler {
    ...
    fn local_module_path(&self, u: &UseDecl, base_dirs: &[PathBuf]) -> Option<PathBuf> {
        ...
        for len in (1..=u.path.len()).rev() {
            prefixes.push(&u.path[0..len]);
        }
        ...
        for rel in prefixes {
            for base in base_dirs {
                ...
                // 2. Directory module: base/seg/mod.dtr, lib.dtr, src/lib.dtr
                let mut lib_dir = base.clone();
                for seg in rel {
                    lib_dir.push(seg);
                }
                let candidate_mod = lib_dir.join("mod.dtr");
                ...
                // 3. Subdirectories lib/, packages/, modules/: base/container/seg...
                for container in &["lib", "packages", "modules"] {
                    let mut cont_dir = base.join(container);
                    for seg in rel {
                        cont_dir.push(seg);
                    }
                    let mut c_file = cont_dir.clone();
                    c_file.set_extension("dtr");
                    if c_file.is_file() {
                        return Some(c_file);
                    }
                    ...
                    if let Ok(pkg_json) = std::fs::read_to_string(cont_dir.join("package.json")) {
                        if let Ok(pkg_data) = serde_json::from_str::<serde_json::Value>(&pkg_json) {
                            if let Some(entry_str) = pkg_data.get("entry").and_then(|e| e.as_str())
                            {
                                let c_entry = cont_dir.join(entry_str);
                                if c_entry.is_file() {
                                    return Some(c_entry);
                                }
                            }
                    ...
                    if cont_dir.is_dir()
                        && let Ok(entries) = std::fs::read_dir(&cont_dir)
                    {
                        let dtr_files: Vec<PathBuf> = entries
                            .flatten()
                            .map(|e| e.path())
                            .filter(|p| {
                                p.is_file() && p.extension().and_then(|s| s.to_str()) == Some("dtr")
                            })
                            .collect();
                        if dtr_files.len() == 1 {
                            return Some(dtr_files[0].clone());
                        }
                    }
                }
            }
    ...
    pub(super) fn resolve_modules(
        ...
        if !program.file.is_empty() {
            ...
            let mut root_file_deps = Vec::new();
            for decl in &program.declarations {
                if let Decl::Use(u) = decl {
                    ...
                    if let Some(path) = path {
                        root_file_deps.push(path.canonicalize().unwrap_or_else(|_| path.clone()));
                    }
                }
            }
            deps.push((root_canon, root_file_deps));
        }
        ...
        loop {
            ...
            for decl in &program.declarations {
                if let Decl::Use(u) = decl {
                    ...
                    if first_seg == Some("python") && u.path.len() > 1 {
                    ...
                    if first_seg == Some("rust") && u.path.len() > 1 {
                            ...
                            && checked_rust_crates.insert(rust_crate.to_string())
                        {
                            ...
                            let crate_exists = Path::new(&format!("{}.dll", rust_crate)).exists()
                                || Path::new(&format!("target/release/{}.dll", rust_crate))
                                    .exists()
                                || Path::new(&format!("tests/fixtures/{}/Cargo.toml", rust_crate))
                                    .exists()
                                || Path::new(&format!("crates/{}/Cargo.toml", rust_crate)).exists()
                                || Path::new(&format!("{}/Cargo.toml", rust_crate)).exists()
                                || cargo_has_dep;
                            ...
                            if !crate_exists && !KNOWN_RUST_CRATES.contains(&rust_crate) {
                                diag.error(
                                    ErrorCode::ResolveUnreachableModule,
                                    format!(
            ...
            for (file, span) in to_load {
                ...
                let has_exports = sub.declarations.iter().any(|d| match d {
                ...
                if has_exports {
                    for decl in &program.declarations {
                        if let Decl::Use(u) = decl {
                            if let Some(target_path) = self.local_module_path(u, &base_dirs) {
                                let target_canon = target_path
                                    .canonicalize()
                                    .unwrap_or_else(|_| target_path.clone());
                                if target_canon == file && u.path.len() >= 2 {
                                    let item_name = u.path.last().unwrap();
                                    let mod_name = u.path[..u.path.len() - 1].join(".");
                                    for d in &sub.declarations {
                                        let (d_name, is_export) = match d {
                                            Decl::Class(c) => (Some(&c.name), c.is_export),
                                            Decl::Enum(e) => (Some(&e.name), e.is_export),
                                            Decl::Function(f) => (Some(&f.name), f.is_export),
                                            Decl::Component(c) => (Some(&c.name), c.is_export),
                                            Decl::Role(r) => (Some(&r.name), r.is_export),
                                            Decl::Trait(t) => (Some(&t.name), t.is_export),
                                            Decl::Type(td) => (Some(&td.name), td.is_export),
                                            _ => (None, false),
                                        };
                                        if let Some(name) = d_name {
                                            if name == item_name && !is_export {
                                                diag.error(
                                                    ErrorCode::PrivateItemAccess,
                                                    format!("Cannot access private item '{}' from module '{}'", item_name, mod_name),
                                                    Some(u.span.clone()),
                                                );
                                            }
                                        }
                ...
                let is_stdlib = file.to_string_lossy().contains("stdlib");
                for mut d in sub.declarations {
                    if is_stdlib || !has_exports {
                        match &mut d {
                    ...
                    let is_dup = match &d {
                        ...
                        Decl::Trait(t) => program.declarations.iter().any(|existing| {
                            if let Decl::Trait(et) = existing {
                                et.name == t.name
                            } else {
                                false
                            }
                        }),
                        Decl::Impl(i) => program.declarations.iter().any(|existing| {
                            if let Decl::Impl(ei) = existing {
                                ei.trait_name == i.trait_name && ei.target_type == i.target_type
                            } else {
                                false
                            }
                        }),
