...
impl LoopOptimizer {
    ...
    fn apply_fold(f: &mut Function, plan: &FoldPlan, trace: &mut OptimizationDecisionTrace) {
        ...
        let (_closed, s_final) = if plan.is_float {
            ...
            let closed = match plan.term {
                ...
                // INVARIANT: Guarded by earlier check verifying term is FloatInvariantConst
                _ => unreachable!("guaranteed by float closed-form plan classification"),
            };
            ...
            let closed = match plan.term {
                ...
                // INVARIANT: Guarded by earlier check verifying term is an integer linear recurrence or piecewise
                _ => unreachable!("guaranteed by integer closed-form plan classification"),
            };
    ...
    pub fn bce_pass(
        ...
        for lp in &cfg.loops {
            ...
            // The step block must be a latch: it terminates with an unconditional
            ...
            // Safe induction step optimization: the loop counter starts at 0, steps by 1 in the latch,
            // and is strictly bounded by `i < bound`. It cannot overflow i64, so rewrite `i + 1` to wrapping_+.
                ...
                if let Some(block) = f.get_block_mut(block_id) {
                    for (inst_idx, inst) in block.instructions.iter_mut().enumerate() {
                        if let Inst::Call { func, args, .. } = inst
                            ...
                            && args.len() >= 2
                        {
                            // If in the same block as the increment step, the access must occur BEFORE the step.
