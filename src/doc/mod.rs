...
/// Collects and parses documentation metadata from Datara files in `target_dir`
pub fn collect_modules(target_dir: &Path) -> Result<Vec<DocModule>, String> {
    let mut modules = Vec::new();
    collect_and_parse_docs(target_dir, &mut modules)?;
    Ok(modules)
}

/// Renders API documentation as GitHub Flavored Markdown
pub fn render_markdown(modules: &[DocModule]) -> String {
    let mut md = String::new();
    md.push_str("# Datara API Documentation\n\n");
    md.push_str("## Modules Overview\n\n");
    for m in modules {
        md.push_str(&format!(
            "- [{}](#module-{})\n",
            m.name,
    ...
    for m in modules {
        ...
        if m.items.is_empty() {
            md.push_str("_No documented items in this module._\n\n");
            continue;
        }
...
/// Generates standalone Markdown documentation from Datara source files
pub fn generate_markdown_docs(target_dir: &Path, output_file: &Path) -> Result<usize, String> {
    let modules = collect_modules(target_dir)?;
    if modules.is_empty() {
        return Err("No Datara (.dtr or .forge) files found to document.".to_string());
    }
    ...
    fs::write(output_file, md).map_err(|e| e.to_string())?;
    Ok(total_items)
}

/// Extracts ```datara code fences from doc comments and validates them using ForgenCompiler check mode
pub fn check_doc_examples(modules: &[DocModule]) -> Result<usize, String> {
    let compiler = crate::driver::ForgenCompiler::new("check");
    let mut checked_count = 0;
    let mut failures = Vec::new();

    for m in modules {
        for item in &m.items {
            let comment = &item.doc_comment;
            let mut in_block = false;
            let mut current_snippet = Vec::new();

            for line in comment.lines() {
                let trimmed = line.trim();
                if trimmed.starts_with("```datara") || trimmed.starts_with("```forge") {
                    in_block = true;
                ...
                if trimmed.starts_with("```") && in_block {
                    ...
                    checked_count += 1;
                    let tag = format!("{}:{}", item.file, item.name);
                    let res = compiler.check_source(&full_code, &tag);
                    if !res.success {
                        failures.push(format!("Doc test failed in {}:\n{}", tag, res.diagnostics));
                    }
                    current_snippet.clear();
                    continue;
                }
...
mod tests {
    ...
    fn test_render_markdown_format() {
        let modules = vec![DocModule {
            name: "math".to_string(),
            path: "stdlib/math.dtr".to_string(),
            items: vec![DocItem {
                name: "add".to_string(),
                kind: "fn".to_string(),
                signature: "fn add(a: Int, b: Int) -> Int".to_string(),
                doc_comment: "Adds two integers.".to_string(),
                file: "stdlib/math.dtr".to_string(),
                effects: vec!["pure".to_string()],
            }],
        }];

        let md = render_markdown(&modules);
        assert!(md.contains("# Datara API Documentation"));
        assert!(md.contains("## Module `math`"));
        assert!(md.contains("### `add` (fn)"));
        assert!(md.contains("```datara\nfn add(a: Int, b: Int) -> Int\n```"));
        assert!(md.contains("**Effects**: `pure`"));
        assert!(md.contains("Adds two integers."));
    }

    #[test]
    fn test_check_doc_examples_success_and_failure() {
        let good_modules = vec![DocModule {
            name: "sample".to_string(),
            path: "sample.dtr".to_string(),
            items: vec![DocItem {
                name: "hello".to_string(),
                kind: "fn".to_string(),
                signature: "fn hello()".to_string(),
                doc_comment: "Example:\n```datara\nlet x = 42\nout x\n```".to_string(),
                file: "sample.dtr".to_string(),
                effects: vec![],
            }],
        }];

        let count = check_doc_examples(&good_modules).expect("Doc check should succeed");
        assert_eq!(count, 1);

        let bad_modules = vec![DocModule {
            name: "broken".to_string(),
            path: "broken.dtr".to_string(),
            items: vec![DocItem {
                name: "bad".to_string(),
                kind: "fn".to_string(),
                signature: "fn bad()".to_string(),
                doc_comment: "Broken example:\n```datara\nlet x = @@@syntax_error@@@\n```"
                    .to_string(),
                file: "broken.dtr".to_string(),
                effects: vec![],
            }],
        }];

        let res = check_doc_examples(&bad_modules);
        assert!(res.is_err());
    }
}
