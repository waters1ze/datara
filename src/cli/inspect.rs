...
pub(crate) fn cmd_inspect(args: &[String]) -> bool {
    ...
    let query = &args[2];
    let mut file_arg = args[3].as_str();
    let mut target_symbol = args.get(4).map(|s| s.as_str());

    // Ergonomic CLI support: allow both `inspect <query> <file.dtr> [symbol]`
    // and `inspect <query> <symbol> <file.dtr>`.
    if let Some(cand_file) = target_symbol {
        if (!Path::new(file_arg).exists() && Path::new(cand_file).exists())
            || (!file_arg.ends_with(".dtr") && cand_file.ends_with(".dtr"))
        {
            target_symbol = Some(file_arg);
            file_arg = cand_file;
        }
    }
        ...
        match query.as_str() {
            ...
            "optimize" => {
                if let Some(sym_name) = target_symbol {
                    ...
                    for node in graph.nodes.values() {
                        if node.id.starts_with("fn:") {
                            fn_names.push(node.label.clone());
                        }
                    }
                    fn_names.sort();
                    if fn_names.len() == 1 {
                        if let Some(opt) = graph.inspect_optimization(&fn_names[0]) {
                            println!("{}", to_pretty_json(&opt));
                        }
                    } else {
                        let mut all_opts = serde_json::Map::new();
                        for name in &fn_names {
                            if let Some(opt) = graph.inspect_optimization(name) {
                                all_opts.insert(name.clone(), opt);
                            }
                        }
                        println!("{}", to_pretty_json(&serde_json::Value::Object(all_opts)));
                    }
                }
            }
            "dependencies" => {
            ...
            "asm" | "clif" => {
                if let Some(clif) = res.clif_source {
                    println!("{}", clif);
                } else if let (Some(dmir), Some(prog)) = (&res.dmir_module, res.program.as_ref()) {
                    let cranelift_backend = crate::codegen::cranelift::CraneliftBackend::for_host();
                    let resolver = crate::resolver::Resolver::new();
                    let types = crate::types::TypeChecker::new(&resolver);
                    let clif = cranelift_backend
                        .emit_clif(dmir, prog, &types)
                        .unwrap_or_else(|e| format!("; CLIF ERROR [E0903]: {}\n", e));
                    println!("{}", clif);
                }
            }
            _ => {
