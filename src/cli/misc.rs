...
Project Commands:
  ...
  context <symbol> [tgt]  Semantic Context API providing structured semantic metadata (JSON)
...
pub fn write_zip(path: &Path, entries: &[(String, Vec<u8>)]) -> Result<(), String> {
    ...
    if entries.len() > u16::MAX as usize {
        return Err("Too many entries for standard ZIP archive".into());
    }

    for (name, data) in entries {
        let name_bytes = name.as_bytes();
        if name_bytes.len() > u16::MAX as usize || data.len() > u32::MAX as usize {
            return Err(format!("Entry '{}' exceeds ZIP format limits", name));
        }
        let crc = crc32(data);
