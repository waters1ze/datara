...
/// `forgen doc` — generate single-file SPA or Markdown documentation, or check doc examples.
pub(crate) fn cmd_doc(args: &[String]) -> bool {
    let is_open = args.iter().any(|a| a == "--open");
    let is_check = args.iter().any(|a| a == "--check");
    let is_markdown = args.iter().any(|a| a == "--markdown");
    let target_opt = args
        .iter()
        .skip(2)
        .find(|a| !a.starts_with('-'))
    ...
    if is_check {
        println!(
            "[Forgen doc] Checking doc examples in '{}'...",
            search_path.display()
        );
        match crate::doc::collect_modules(search_path) {
            Ok(modules) => match crate::doc::check_doc_examples(&modules) {
                Ok(count) => {
                    println!(
                        "[Forgen doc] All {} doc example(s) passed check successfully.",
                        count
                    );
                }
                Err(err) => {
                    eprintln!("[Forgen doc] Doc example verification failed:\n{}", err);
                    std::process::exit(1);
                }
            },
            Err(e) => {
                eprintln!("Doc check error: {}", e);
                std::process::exit(1);
            }
        }
        return true;
    ...
    if is_markdown {
        let out_file = Path::new("target").join("doc").join("index.md");
        println!(
            "[Forgen doc] Scanning '{}' and building Markdown documentation...",
            search_path.display()
        );
        match crate::doc::generate_markdown_docs(search_path, &out_file) {
            Ok(count) => {
                println!(
                    "[Forgen doc] Generated Markdown documentation for {} item(s) at: {}",
                    count,
                    out_file.display()
                );
            }
            Err(e) => {
                eprintln!("Markdown doc generation error: {}", e);
                std::process::exit(1);
            }
        }
        return true;
