...
impl SemanticGraph {
    ...
    pub fn build(_program: &Program, resolver: &Resolver, effects: &EffectAnalyzer) -> Self {
        ...
        // 1. Add classes in deterministic order
        let mut sorted_classes: Vec<(&String, &crate::resolver::Symbol)> =
            resolver.classes.iter().collect();
        sorted_classes.sort_by_key(|(name, _)| *name);

        for (name, sym) in sorted_classes {
            let mut field_names: Vec<&String> = sym.fields.keys().collect();
            field_names.sort();
            ...
            let mut method_names: Vec<&String> = sym.methods.keys().collect();
            method_names.sort();
            ...
            meta.insert(
                "compositions".into(),
                serde_json::to_value(&sym.compositions).unwrap_or_default(),
            );
        ...
        // 2. Add functions in deterministic order
        let mut sorted_functions: Vec<(&String, &crate::resolver::Symbol)> =
            resolver.functions.iter().collect();
        sorted_functions.sort_by_key(|(name, _)| *name);

        for (name, sym) in sorted_functions {
            let eff_str = effects
                .function_effects
                .get(name)
                .map(|e| e.to_string())
                ...
                GraphNode {
                    ...
                    optimization: OptimizationFacts {
                        inlining: InliningFact {
                            ...
                            } else if eff_str.contains("Foreign")
                                || eff_str.contains("Nondeterministic")
                            {
                                "Foreign or non-deterministic call; must not be inlined or folded"
                                    .into()
                            } else {
    ...
    pub fn attach_optimization_report(
        ...
        for (node_id, node) in self.nodes.iter_mut() {
            ...
            if !fn_key.is_empty() {
                ...
                if let Some(own) = report.ownership_reports.get(&fn_key) {
                    node.optimization.ownership = Some(own.clone());
                } else if let Some(ref own) = report.ownership {
                    node.optimization.ownership = Some(own.clone());
                }
            }
        }
    ...
    pub fn find_symbol(&self, name: &str) -> Option<&GraphNode> {
        ...
        let fn_id = format!("fn:{}", name);
        if let Some(node) = self.nodes.get(&fn_id) {
            return Some(node);
        }
        let class_id = format!("class:{}", name);
        ...
        for node in self.nodes.values() {
            if node.label.ends_with(&format!(".{}", name))
    ...
    pub fn inspect_optimization(&self, name: &str) -> Option<serde_json::Value> {
        ...
        let is_det =
            !node.effects.contains("Nondeterministic") && !node.effects.contains("Foreign");
        root.insert("isDeterministic".into(), serde_json::Value::Bool(is_det));
        root.insert(
            "effects".into(),
            serde_json::Value::String(node.effects.clone()),
        );
        if let Some(ref own) = node.optimization.ownership {
            root.insert("ownership".into(), serde_json::Value::String(own.clone()));
        }
        root.insert(
