...
pub fn verify_ed25519_signature(message: &[u8], sig_hex: &str, pk_hex: &str) -> Result<(), String> {
    ...
    let pk = ed25519_compact::PublicKey::from_slice(&pk_bytes)
    ...
    pk.verify(message, &sig).map_err(|_| {
        "Cryptographic signature verification failed: ed25519 signature mismatch".to_string()
    })
}

pub fn create_ed25519_keypair() -> (String, String) {
    let kp = ed25519_compact::KeyPair::generate();
    (bytes_to_hex(kp.pk.as_ref()), bytes_to_hex(kp.sk.as_ref()))
}

pub fn sign_ed25519_message(message: &[u8], sk_hex: &str) -> Result<String, String> {
    let sk_bytes = hex_to_bytes(sk_hex)?;
    let sk = ed25519_compact::SecretKey::from_slice(&sk_bytes)
    ...
    match cmd {
        ...
        "add" => {
            ...
            if pkg_name.starts_with("sparks/") {
                ...
                match registry.fetch_and_install_sparks(&pkg_name, None, &reg_url, current_dir) {
                    Ok(path) => {
                        println!("[DONE] Installed {} -> {}", pkg_name, path.display());
        ...
        "install" | "i" | "restore" => {
            if let Some(target) = args.get(2) {
                if target.starts_with("sparks/") {
                    ...
                    match registry.fetch_and_install_sparks(target, None, &reg_url, current_dir) {
                        Ok(path) => {
                            println!("[DONE] Installed {} -> {}", target, path.display());
                            println!("[OK] Cryptographic ed25519 signature & SHA-256 verified.");
                            return;
                        }
                        Err(e) => {
