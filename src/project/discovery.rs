...
impl ProjectDiscovery {
    ...
    pub fn discover(target: Option<&Path>) -> Result<ProjectLayout, String> {
        ...
        if source_files.is_empty() && test_files.is_empty() && example_files.is_empty() {
            return Err(format!("No .dtr source files found in '{}'", dir.display()));
        }

        // Determine entry point (main.dtr prioritized, else first file, else first test or example)
        let entry_point = if let Some(idx) = source_files
            .iter()
            .position(|p| p.file_name().and_then(|n| n.to_str()) == Some("main.dtr"))
        {
            ...
            main_file
        } else if let Some(first_src) = source_files.first() {
            first_src.clone()
        } else if let Some(first_test) = test_files.first() {
            first_test.clone()
        } else {
            example_files[0].clone()
        };
