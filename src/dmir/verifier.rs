...
pub fn verify_function(function: &Function) -> Result<(), String> {
    ...
    for block in &function.blocks {
        ...
        for (index, instruction) in block.instructions.iter().enumerate() {
            if let Inst::InlineAsm { outputs, .. } = instruction {
            ...
            } else if let Some(dest) = instruction_dest(instruction) {
                define_value(
                    &mut definitions,
                    dest,
                    block.id,
...
fn check_use(
    ...
    if def_block == block {
        ...
        if ok {
            ...
            return Err(format!(
                "value {} used in block {} before its definition in the same block",
                value.0, block.0
            ));
        }
    }
