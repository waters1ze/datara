...
#[test]
fn test_stdlib_trig_and_clamp_and_time_ns() {
    let code = r#"
fn main() {
    let sn = math_sin(0.0)
    let cs = math_cos(0.0)
    let tn = math_tan(0.0)
    let cl1 = math_clamp(15.0, 0.0, 10.0)
    let cl2 = math_clamp(-5.0, 0.0, 10.0)
    let cl3 = math_clamp(7.5, 0.0, 10.0)
    let cli1 = math_clamp_int(50, 10, 30)
    let cli2 = math_clamp_int(5, 10, 30)
    let cli3 = math_clamp_int(20, 10, 30)
    let pj = path_join("usr", "bin")
    let t_ms = now_ms()
    let t_ns = now_ns()
...
"#;
    let out = run_datara(code, "test_stdlib_trig_clamp");
    let lines: Vec<&str> = out.lines().collect();
    assert_eq!(lines[0], "0");
    assert_eq!(lines[1], "1");
    assert_eq!(lines[2], "0");
    ...
    assert_eq!(lines[4], "0");
    assert_eq!(lines[5], "7.5");
    assert_eq!(lines[6], "30");
    assert_eq!(lines[7], "10");
    assert_eq!(lines[8], "20");
    assert!(lines[9].contains("usr") && lines[9].contains("bin"));
