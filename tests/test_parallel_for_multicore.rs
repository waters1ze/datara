...
"#;
    ...
    assert!(out.contains("PAR_MS:"), "Output must contain PAR_MS tag");
    assert!(out.contains("SEQ_MS:"), "Output must contain SEQ_MS tag");
    assert!(
        seq_ms > 0,
        "Sequential execution time must be measured (>0 ms)"
    );
    assert!(
        par_ms > 0,
        "Parallel execution time must be measured (>0 ms)"
    );
