//! Phase 8 Test Suite: Bounds-Check Elimination (BCE)
//!
//! Validates:
//! 1. Inductive loop range analysis: Hot loops with List indexing compile WITHOUT runtime
//!    bounds checks (`datara_rt_list_get_unchecked` in LLVM IR).
//! 2. Condition dominator proofs: guarded indexing `if idx < arr.len()` eliminates redundant checks.
//! 3. Negative safety preservation: unproven or out-of-bounds accesses retain runtime checks.
//! 4. Float List BCE: floating-point array indexing eliminates checks cleanly.
...
#[test]
fn test_v120_bce_loop_indexing_elimination_and_report() {
    let source = r#"
...
#[test]
fn test_v120_bce_condition_dominator_elimination() {
    let source = r#"
...
#[test]
fn test_v120_bce_negative_safety_preservation() {
    let source = r#"
...
"#;
    ...
    assert!(
        llvm.contains("call i64 @datara_rt_list_get(")
            ...
            || llvm.contains("@datara_rt_list_get"),
...
#[test]
fn test_v120_bce_float_list_indexing() {
    let source = r#"
fn sum_floats(arr: List<Float>) -> Float {
    mut total: Float = 0.0
    mut i = 0
    let n = arr.len()
    while i < n {
...
fn main() {
    val items = [1.5, 2.5, 3.5, 4.5]
    val res = sum_floats(items)
    println(res)
}
"#;

    let compiler = ForgenCompiler::new("domain").with_llvm(true);
    let res = compiler.compile_source(source, "bce_float_sum.dtr", None);

    assert!(res.success, "Compilation must succeed: {:?}", res.error);
    let llvm = res.llvm_source.expect("LLVM IR must be generated");
    assert!(
        llvm.contains("datara_rt_list_get_unchecked"),
        "Float indexing must also benefit from BCE unchecked get"
    );
...
#[test]
fn test_v120_bce_array_sum_microbenchmark_safe_datara_vs_unsafe_c() {
