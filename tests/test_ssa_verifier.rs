...
#[test]
fn verifier_rejects_same_block_use_before_def() {
    // Instruction 0 uses %1, but %1 is only defined at instruction 1 in the same block.
    let f = Function {
        name: "same_block_early_use".into(),
        params: vec![],
        return_type: "Int".into(),
        entry_block: BasicBlockId(0),
        blocks: vec![block(
    ...
    let err = verify_function(&f).expect_err("same-block use before def must be rejected");
    assert!(
        err.contains("before its definition in the same block"),
        "got: {}",
        err
    );
}
...
#[test]
fn verifier_rejects_duplicate_function_parameter() {
    let f = Function {
        name: "dup_params".into(),
        params: vec![
            ("a".into(), "Int".into(), ValueId(0)),
            ("b".into(), "Int".into(), ValueId(0)),
        ],
        return_type: "Int".into(),
        entry_block: BasicBlockId(0),
        blocks: vec![block(
    ...
    let err = verify_function(&f).expect_err("duplicate param ValueId must be rejected");
    assert!(err.contains("defined more than once"), "got: {}", err);
}
