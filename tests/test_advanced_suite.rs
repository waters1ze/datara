...
fn test_lsp_server_protocol_lifecycle() {
    ...
    let out_hover = String::from_utf8(output.clone()).expect("valid utf8");
    assert!(
        out_hover.contains("Datara Semantic Inspector"),
        "Hover must contain semantic inspector"
    );
    assert!(
        out_hover.contains("100.0% proven (zero-cost)"),
        "Hover must contain ownership stats"
    );

    // 3b. Test hover on user function and local variable
    output.clear();
    let doc_open = serde_json::from_str::<serde_json::Value>(
        r#"{
        "jsonrpc": "2.0",
        ...
        "params": {
            "textDocument": {
                "uri": "app.dtr",
                "text": "fn calculate_total(price: Int) -> Int {\n    let discount = 5\n    return price - discount\n}"
            }
        }
    }"#,
    )
    .unwrap();
    server
        .handle_request(&serde_json::from_value(doc_open).unwrap(), &mut output)
        .expect("didOpen");

    output.clear();
    let hover_fn = serde_json::from_str::<serde_json::Value>(
        r#"{
        "jsonrpc": "2.0",
        "id": 4,
        "method": "textDocument/hover",
        "params": {
            "textDocument": { "uri": "app.dtr" },
    ...
    .unwrap();
    server
        .handle_request(&serde_json::from_value(hover_fn).unwrap(), &mut output)
        .expect("hover fn");
    let out_fn_hover = String::from_utf8(output).expect("valid utf8");
    assert!(out_fn_hover.contains("calculate_total"));
    assert!(out_fn_hover.contains("Ownership: 100.0% proven (zero-cost)"));
}
