...
#[test]
fn test_map_probe_large_literal_and_float_values() {
    let out = run_datara(
        r#"
fn main() {
    // 1. Literal with >5 entries (triggers oversized fallback)
    mut m = [
        "one": 1,
        "two": 2,
        "three": 3,
    ...
    // 2. Map with float values (F64 bitcast coercion)
    let fmap = ["pi": 3.14, "e": 2.71]
    out fmap["pi"]
    out fmap["e"]
}
"#,
    ...
    assert_eq!(lines[1], "7");
    assert_eq!(lines[2], "8");
    assert!(lines[3].starts_with("3.14"));
    assert!(lines[4].starts_with("2.71"));
}
