...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$p = 'payload'; if (Test-Path $p) { rm -r -fo $p }; md $p\bin,$p\assets | Out-Null; cp ..\target\release\forgen.exe $p\bin\ -fo; cp ..\target\release\datara.exe $p\bin\ -fo; cp ..\target\release\dpm.exe $p\bin\ -fo; cp ..\assets\datara.ico $p\assets\ -fo; cp ..\assets\datara-logo.png $p\assets\ -fo; cp ..\stdlib $p\ -r -fo; cp ..\runtime $p\ -r -fo; cp ..\editors $p\ -r -fo; if (Test-Path payload.zip) { rm payload.zip -fo }; Compress-Archive -Path $p\* -DestinationPath payload.zip -CompressionLevel Optimal; rm -r -fo $p"
